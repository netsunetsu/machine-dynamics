...
def LoaderNormalizer(data, isTest = False, shuffle = 0, dataProp = None, res=256):
    """
    ...
    if dataProp is None:
        ...
        data.inputs  = np.empty((len(files), 3, res, res))
        data.targets = np.empty((len(files), 3, res, res))
        ...
        for i, file in enumerate(files):
            ...
            #for j in range(2):
               #for k in range(10):
                  #for l in range(10):
                        #add = np.random.random_sample()
                        #data.inputs[i][j][k][l] += add
            data.targets[i] = d[3:6]
        print("\n" "Number of data loaded:", len(data.inputs) )
       

    else:
        # load from folders reg, sup, and shear under the folder dataDir
        data.totalLength = int(dataProp[0])
        data.inputs  = np.empty((data.totalLength, 3, res, res))
        data.targets = np.empty((data.totalLength, 3, res, res))
...
class TurbDataset(Dataset):
    ...
    def __init__(self, dataProp=None, mode=TRAIN, dataDir="../data/128/train/", 
dataDirTest="../data/256/test/", shuffle=0, normMode=0, res=256):
        ...
        # load & normalize data
        self = LoaderNormalizer(self, isTest=(mode==self.TEST), dataProp=dataProp, shuffle=shuffle, res=res)
